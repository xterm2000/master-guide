...
| `ssh/` | key generation/distribution, SSH certificate authority (`ssh-keygen -s`, KRL revocation), passwordless sudo, mass reboot, node IP map, SSH client config, node-connect script, Windows key perms — see [`ssh/`](ssh/) |
